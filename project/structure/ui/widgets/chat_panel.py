...
class ChatPanel(QWidget):
    ...
    clear_requested = Signal()  # Signal émis quand l'utilisateur demande d'effacer la conversation
    project_type_selected = Signal(str)  # Signal émis quand un type de projet est sélectionné
    technology_selected = Signal(str, str)  # Signal émis lorsqu'une technologie est sélectionnée (tech_id, project_type_id)
    language_selected = Signal(str, str)  # Signal émis lorsqu'un langage est sélectionné (lang_id, tech_id)
    project_name_submitted = Signal(str)  # Signal émis lorsqu'un nom de projet est soumis
    ...
    def add_user_message(self, text):
        ...
        container_layout.addStretch(1)  # Espace extensible à gauche pour pousser vers la droite
    ...
    def add_ai_message(self, message, word_wrap=True, icon_name="user", icon_color="#2196F3", icon_size=20):
        ...
        if not word_wrap:
            ...
            label.setMaximumWidth(400)  # Limiter la largeur pour éviter les lignes trop longues
    ...
    def display_project_types(self):
        ...
        try:
                ...
                if hasattr(self, "_on_technology_selected"):
                    ...
                    try:
                        ...
                        if hasattr(self.project_show, "technology_selected"):
                            ...
                            if QObject.receivers(self.project_show.technology_selected) > 0:
                                self.project_show.technology_selected.disconnect(self._on_technology_selected)
                    except Exception as e:
                        print(f"[INFO] Signal déjà déconnecté ou jamais connecté: {str(e)}")
                    
                    # Rétablir l'état des signaux
                    self.project_show.blockSignals(old_state)
                    
                    # Connecter le signal (est toujours sécurisé)
                    self.project_show.technology_selected.connect(self._on_technology_selected)
            ...
            self.project_types_grid.project_type_selected.connect(self.on_project_type_selected)
    ...
    def display_technologies(self, technologies, project_type_id, project_type_name, project_color):
    ...
    def _on_technology_selected(self, tech_id, project_type_id):
        ...
        if hasattr(self, "technologies_grid") and self.technologies_grid is not None:
            try:
                ...
                self.technologies_grid.technology_selected.disconnect(self._on_technology_selected)
                self.technologies_grid.back_requested.disconnect(self._back_to_project_types)
                ...
                print(f"[ATTENTION] Erreur lors du nettoyage du composant technologies_grid: {str(e)}")
    ...
    def _back_to_project_types(self):
        ...
        if hasattr(self, "technologies_grid") and self.technologies_grid is not None:
            try:
                ...
                print(f"Erreur lors du nettoyage de la grille de technologies: {str(e)}")
    ...
    def display_languages(self, tech_id, project_type_id):
        ...
        from project.structure.ui.widgets.programming_languages_grid import ProgrammingLanguagesGrid
        ...
        self.languages_grid.back_requested.connect(lambda: self._back_to_technologies(project_type_id))
    ...
    def add_ai_infos(self, message, word_wrap=True, icon_name="info", icon_color="#64B5F6", icon_size=20):
    ...
    def _on_language_selected(self, lang_id, tech_id):
        ...
        if hasattr(self, "languages_grid") and self.languages_grid is not None:
            try:
                # Déconnecter les signaux
                self.languages_grid.language_selected.disconnect(self._on_language_selected)
                ...
                print(f"[ATTENTION] Erreur lors du nettoyage du composant languages_grid: {str(e)}")
        
        # Émettre le signal pour la classe parente

        # Affiche un recap des infos
        self.add_ai_infos(f"<b>Information:</b><br>Vous avez choisi <b>{tech_id}</b> pour la technologie<br>et <b>{lang_id}</b> pour le langage de programmation.")
    ...
    def _back_to_technologies(self, project_type_id):
        ...
        if hasattr(self, "languages_grid") and self.languages_grid is not None:
            try:
                ...
                print(f"[DEBUG] Erreur lors du nettoyage du composant languages_grid: {str(e)}")
        ...
        try:
            ...
            technologies = project_show.get_technologies_for_project_type(project_type_id)
            ...
            print(f"[DEBUG] Retour aux technologies pour {project_name} ({project_type_id}): {len(technologies)} technologies")
    ...
    def _back_to_help_cards(self):
        ...
        if hasattr(self, "project_types_grid") and self.project_types_grid is not None:
            try:
                ...
                print(f"Erreur lors du nettoyage de la grille de types de projet: {str(e)}")
        ...
        try:
            ...
            if hasattr(self, "project_show") and self.project_show is not None:
                try:
                    ...
                    print(f"Erreur lors de la récupération des données du type de projet: {str(e)}")
            
            # Afficher les technologies pour ce type de projet
            self.display_technologies(technologies, project_type_id, project_name, project_color)
