...
class CustomWebPage(QWebEnginePage):
    ...
    def javaScriptConsoleMessage(self, level, message, lineNumber, sourceID):
        ...
        print(
            f"JavaScript {level_str}: {message} (ligne {lineNumber}, source: {sourceID})"
        )
...
class DocumentationWidget(QWidget):
    def __init__(self, doc_type: DocType):
        ...
        self.thread_pool.setMaxThreadCount(
            2
        )  # Augmenté pour permettre plusieurs générations
    ...
    def _init_lazy_components(self):
        ...
        # Vues web
        self.html_view_component = LazyLoadedComponent(lambda: self._create_web_view())
    ...
    def setup_body(self):
        ...
        self.progress_bar.setStyleSheet(
            ...
            QProgressBar::chunk {
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #67E5FF, stop:1 #1E90FF);
                border-radius: 2px;
            }
            QProgressBar[value="50"], QProgressBar[value="51"], QProgressBar[value="52"],
            QProgressBar[value="53"], QProgressBar[value="54"], QProgressBar[value="55"],
            QProgressBar[value="56"], QProgressBar[value="57"], QProgressBar[value="58"], QProgressBar[value="59"],
            QProgressBar[value="60"], QProgressBar[value="61"], QProgressBar[value="62"],
            QProgressBar[value="63"], QProgressBar[value="64"], QProgressBar[value="65"],
            QProgressBar[value="66"], QProgressBar[value="67"], QProgressBar[value="68"], QProgressBar[value="69"],
            QProgressBar[value="70"], QProgressBar[value="71"], QProgressBar[value="72"],
            QProgressBar[value="73"], QProgressBar[value="74"], QProgressBar[value="75"],
            QProgressBar[value="76"], QProgressBar[value="77"], QProgressBar[value="78"], QProgressBar[value="79"],
            QProgressBar[value="80"], QProgressBar[value="81"], QProgressBar[value="82"],
            QProgressBar[value="83"], QProgressBar[value="84"], QProgressBar[value="85"],
            QProgressBar[value="86"], QProgressBar[value="87"], QProgressBar[value="88"], QProgressBar[value="89"],
            QProgressBar[value="90"], QProgressBar[value="91"], QProgressBar[value="92"],
            QProgressBar[value="93"], QProgressBar[value="94"], QProgressBar[value="95"],
            QProgressBar[value="96"], QProgressBar[value="97"], QProgressBar[value="98"], QProgressBar[value="99"],
    ...
    def _handle_item_click(self, path):
        """Fonction commune pour gérer le clic sur un élément"""
        if path:
    ...
    def handle_tree_click(self, index):
        ...
        if self._handle_item_click(self.current_item_path):
    ...
    def load_content(self, path):
        ...
        if html:
            ...
            formatted_path = path
            if ">" in path:
                formatted_path = path.split(">")[-1].strip()
    ...
    def _prepare_prompt(self, path):
        """Prépare le prompt pour la génération de contenu"""
        ...
        if custom_prompt:
            ...
            section_title = (
                path.split("<i class='nav-icon'></i>")[-1].strip()
                if "<i class='nav-icon'></i>" in path
                else path
            )
            section_name = section_title.split(">")[-1].strip()

            # Cas spécial pour l'historique de l'organisation
            if "Historique de l'organisation" in section_name:
                return f"""
                Tu es un assistant expert en documentation projet.
                Génère un contenu HTML structuré pour la section : {section_name}
                ...
                **Format :**
                - Titre h2 {section_name} avec l'icône 'clock-9' de Lucide (sans mentionner Lucide)
                - Contenu technique concis
                - OBLIGATOIREMENT inclure un diagramme Mermaid de type Gantt avec les années (format YYYY)
                ...
                **Interdictions :**
                ...
                - Ne jamais ajouter ```html en premier ou ``` en dernier
                """
            else:
                return build_prompt(self.doc_type.value, section_name)

    def _update_ui_for_generation(self, path, is_streaming=False):
        """Met à jour l'interface utilisateur pour la génération"""
    ...
    def generate_content(self):
        ...
        prompt = self._prepare_prompt(path)
        ...
        self._update_ui_for_generation(path)

    def _save_version_and_update_content(self, path, html, is_streaming=False):
        """Fonction commune pour sauvegarder une version et mettre à jour le contenu"""
        ...
        if self.current_item_path == path:
            ...
            if is_streaming:
                ...
                self.status_label.setText(
                    f"Génération en streaming terminée pour: {path}"
                )
            else:
    ...
    def on_generation_finished(self, path, html):
        self._save_version_and_update_content(path, html, is_streaming=False)
    ...
    def on_generation_progress(self, progress):
        ...
        # Changer la couleur du texte en blanc lorsque la valeur atteint 50%
        if progress >= 50:
            self.progress_bar.setStyleSheet(
                """
                QProgressBar {
    ...
    def generate_content_streaming(self):
        ...
        if not hasattr(self, "current_item_path") or not self.current_item_path:
            QMessageBox.warning(
                ...
                "Veuillez sélectionner une section dans la table des matières.",
            )
            return
        ...
        self.current_streaming_path = path.split(">")[-1].strip()
        self.is_streaming = True

        # Préparer le prompt en utilisant la fonction utilitaire
        prompt = self._prepare_prompt(self.current_streaming_path)
        ...
        # Mettre à jour l'interface utilisateur en utilisant la fonction utilitaire
        self._update_ui_for_generation(path, is_streaming=True)

    def on_streaming_finished(self, path, html):
        """Gère la fin de la génération en streaming"""
        self._save_version_and_update_content(path, html, is_streaming=True)
    ...
    def on_streaming_chunk(self, chunk):
        ...
        # content_to_display = re.sub(r'^\s*html\s*', '', chunk)
        content_to_display = chunk.replace("html", "")
        content_to_display = content_to_display.replace(r"```", "")
        # Supprimer les balises ``` à la fin
        # content_to_display = re.sub(r'```+\s*$', '', chunk)
        self.streaming_content += content_to_display

        # Afficher le contenu dans la vue
        self.html_view.setHtml(
            render_html(self.streaming_content, self.default_css, skip_title=True)
        )

        # Mettre à jour l'éditeur de contenu avec le contenu brut
    ...
    def apply_content_edits(self):
        ...
        if self.current_item_path in self.generated_content:
            ...
            self.versions[self.current_item_path][self.current_version] = (
                self.generated_content[self.current_item_path]
            )
            self.current_version += 1
        ...
        self.status_label.setText(
            f"Modifications appliquées pour: {self.current_item_path}"
        )
    ...
    def _load_json_file(self, file_type):
        """Fonction utilitaire pour charger un fichier JSON"""
        try:
            file_path = self.save_dir / f"{file_type}_{self.doc_type.name}.json"
            if file_path.exists():
                with open(file_path, "r", encoding="utf-8") as f:
                    return json.load(f)
        except Exception as e:
            print(f"Erreur lors du chargement de {file_type}: {e}")
        return [] if file_type in ["favorites", "history"] else {}

    def _save_json_file(self, file_type, data):
        ...
        try:
            ...
            print(f"Erreur lors de la sauvegarde de {file_type}: {e}")

    def load_favorites(self):
        return self._load_json_file("favorites")

    def save_favorites(self):
        self._save_json_file("favorites", self.favorites)

    def load_history(self):
        return self._load_json_file("history")

    def save_history(self):
        self._save_json_file("history", self.history)

    def load_custom_prompts(self):
        return self._load_json_file("prompts")

    def save_custom_prompts(self):
        self._save_json_file("prompts", self.custom_prompts)
    ...
    def customize_prompt(self):
        ...
        current_prompt = self.custom_prompts.get(
            ...
            build_prompt(
                self.doc_type.value,
                (
                    self.current_item_path.split("<i class='nav-icon'></i>")[-1].strip()
        ...
        if dialog.exec():
            ...
            self.status_label.setText(
                f"Prompt personnalisé pour: {self.current_item_path}"
            )
    ...
    def show_toc_context_menu(self, position):
        ...
        if path in self.favorites:
            ...
            add_favorite_action.triggered.connect(
                lambda: self.add_to_favorites_path(path)
            )
            menu.addAction(add_favorite_action)

        customize_prompt_action = QAction("Personnaliser le prompt", self)
        customize_prompt_action.triggered.connect(
            lambda: self.customize_prompt_for_path(path)
        )
    ...
    def export_to_pdf(self):
        ...
        if ">" in self.current_item_path:
            filename = (
                self.current_item_path.split("<i class='nav-icon'></i>")[-1]
                if "<i class='nav-icon'></i>" in self.current_item_path
                else self.current_item_path
            ).replace(" ", "") + ".pdf"
        else:
        ...
        if file_path:
            try:
                ...
                QMessageBox.warning(
                    ...
                    f"Erreur lors de l'exportation en Word: {e}",
                )
        ...
        if file_path:
            try:
                ...
                QMessageBox.warning(
                    self,
                    "Erreur d'exportation",
                    f"Erreur lors de l'exportation en Markdown: {e}",
                )
        ...
        if file_path:
            try:
                ...
                self.last_save_label.setText(
                    f"Dernière sauvegarde: {data['timestamp']}"
                )
                self.status_label.setText(f"Document sauvegardé: {file_path}")
            except Exception as e:
                QMessageBox.warning(
                    self, "Erreur de sauvegarde", f"Erreur lors de la sauvegarde: {e}"
                )

    def load_document(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Charger un document", str(self.save_dir), "Fichiers JSON (*.json)"
        )
        ...
        if file_path:
            try:
                ...
                QMessageBox.warning(
                    self, "Erreur de chargement", f"Erreur lors du chargement: {e}"
                )
    ...
    def publish_to_git(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                self.status_label.setText(
                    "Préparation des fichiers pour publication..."
                )
                ...
                with open(readme_path, "w", encoding="utf-8") as f:
                    ...
                    f.write(
                        "Ce dépôt contient la documentation générée par AssistantPM.\n\n"
                    )
                ...
                try:
                    ...
                    if credentials["username"]:
                        ...
                        subprocess.run(
                            [
                                ...
                                "config",
                                "user.email",
                                f"{credentials['username']}@users.noreply.github.com",
                            ],
                            cwd=temp_dir,
                            check=True,
                            capture_output=True,
                    ...
                    # Créer un commit
                    commit_message = (
                        f"Publication de la documentation {self.doc_type.value}"
                    )
                    subprocess.run(
                    ...
                    if result.returncode != 0:
                        ...
                        if (
                            "fatal: la branche" in result.stderr
                            or "fatal: The current branch" in result.stderr
                        ):
                            subprocess.run(
                    ...
                    self.status_label.setText(
                        f"Publication réussie sur {credentials['repo']}"
                    )
    ...
    def generate_all_content(self):
        ...
        def collect_paths(parent_index, current_path=""):
            ...
            for row in range(row_count):
                ...
                path = (
                    text
                    if not current_path
                    else f"{current_path} <i class='nav-icon'></i> {text}"
                )
                paths.append(path)
        ...
        for i, path in enumerate(paths):
            ...
            self.status_label.setText(
                f"Génération en cours: {path} ({i+1}/{total_sections})"
            )
        ...
        self.status_label.setText(
            f"Génération complète: {total_sections} sections générées"
        )
